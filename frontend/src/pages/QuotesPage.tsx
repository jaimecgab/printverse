...
import type { Quote, QuoteStatus, QuoteSummary } from '../types'
...
export function QuotesPage() {
  ...
  const [details, setDetails] = useState<Record<number, Quote>>({})
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        if (!active) return
        const loaded: Record<number, Quote> = {}; let failures = 0
        settled.forEach((result) => { if (result.status === 'fulfilled') loaded[result.value.id] = result.value; else failures += 1 })
        setDetails(loaded); setPartial(failures)
  ...
  return (
    <div className="page">
      ...
      {partial > 0 && <div className="inline-notice">La ganancia de {partial} cotización(es) no está disponible; los demás datos permanecen visibles.</div>}
      {filtered.length === 0 ? <EmptyState icon={<FileText />} title={quotes.length ? 'Sin resultados en esta vista' : 'Aún no hay cotizaciones'} description={quotes.length ? 'Ajusta el estado o el texto de búsqueda.' : 'Crea un borrador usando clientes y costos reales.'} action={!quotes.length ? <Link className="button button--primary" to="/quotes/new">Crear primera cotización</Link> : undefined} /> : <section className="quotes-list">{filtered.map((quote) => { const validity = quoteValidity(quote.validUntil); const detail = details[quote.id]; return <Link className="quote-card" to={`/quotes/${quote.id}`} key={quote.id}><div className="quote-stripe" /><div className="quote-main"><div className="quote-heading"><span className="quote-number">{quote.quoteNumber}</span><StatusBadge status={quote.status} /></div><h2>{quote.customer.name}</h2><p>{quote.customer.phone} · Creada {formatDate(quote.createdAt)}</p></div><div className={`validity validity--${validity}`}><span>Vigencia</span><strong>{validity === 'expired' ? 'Vencida' : formatDate(quote.validUntil)}</strong></div><div className="quote-money"><span>Total cotizado</span><strong>{currency.format(quote.total)}</strong><small>{detail ? `Ganancia ${currency.format(detail.estimatedProfit)}` : 'Ganancia no disponible'}</small></div><ArrowRight className="quote-arrow" /></Link>})}</section>}
    </div>
  )
}
