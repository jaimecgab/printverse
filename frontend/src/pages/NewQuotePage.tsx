...
export function NewQuotePage() {
  ...
  const [form, setForm] = useState<QuoteCreateRequest>({ customerId: 0, title: null, validUntil: addDays(15), estimatedDeliveryDate: null, depositPercentage: null, notes: null, internalNotes: null, markupPercentage: 30, discountPercentage: 0, taxEnabled: true, taxPercentage: 16 })
  ...
  async function submit(event: FormEvent) {
    ...
    try {
      const quote = await quotesApi.create({ ...form, title: form.title?.trim() || null, notes: form.notes?.trim() || null, internalNotes: form.internalNotes?.trim() || null })
    ...
    <div className="page page--narrow">
      ...
      {customers.length === 0 ? <div className="empty-state"><h2>Primero necesitas un cliente</h2><p>No usamos datos de ejemplo. Registra un cliente real antes de cotizar.</p><Link to="/customers" className="button button--primary">Ir a clientes</Link></div> : (
        <form className="panel quote-terms-form" onSubmit={submit}>
          ...
          <FormField label="Título del proyecto" htmlFor="quote-title" error={errors.title}><input id="quote-title" maxLength={200} value={form.title || ''} onChange={(event) => setForm({ ...form, title: event.target.value || null })} placeholder="Ej. Trofeos para torneo de verano" /></FormField>
          ...
          <div className="form-grid form-grid--2"><FormField label="Notas para el cliente" htmlFor="quote-notes" error={errors.notes} hint="Se incluirán en el documento comercial."><textarea id="quote-notes" rows={5} maxLength={3000} value={form.notes || ''} onChange={(event) => setForm({ ...form, notes: event.target.value || null })} placeholder="Alcance, colores, condiciones de entrega…" /></FormField><FormField label="Notas internas" htmlFor="quote-internal-notes" error={errors.internalNotes} hint="Sólo para seguimiento del equipo."><textarea id="quote-internal-notes" rows={5} maxLength={3000} value={form.internalNotes || ''} onChange={(event) => setForm({ ...form, internalNotes: event.target.value || null })} placeholder="Decisiones, referencias o pendientes internos…" /></FormField></div>
